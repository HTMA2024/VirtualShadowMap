...
// FALLback
uniform float NGSS_FALLBACK_TO_INTERNAL = 0;
...
// ------------------------------------------------------------------
    ...
    inline fixed UnitySampleShadowmapInternal (float4 shadowCoord)
    {
        #if defined (SHADOWS_SOFT)
    ...
    float2 BLOCKER_SEARCH_SPOT(float4 coord, float diskRadius, float randPied, int samplers, float2 dz_duv)
    {
        ...
        {
            float2 rotatedOffset = VogelDiskSample(i, samplers, randPied) * diskRadius * (i > 0);           
            #if defined(NGSS_USE_SLOPE_BIAS)
    ...
    inline fixed UnitySampleShadowmapNGSS(float4 shadowCoord, float3 screenPos)
    {
        ...
        UNITY_BRANCH
        if (NGSS_FALLBACK_TO_INTERNAL > 0)
        {
            return UnitySampleShadowmapInternal(shadowCoord);
        }
...
// ------------------------------------------------------------------
    ...
    inline half UnitySampleShadowmapInternal (float3 vec)
    {
        #if defined(SHADOWS_CUBE_IN_DEPTH_TEX)
            float3 absVec = abs(vec);
            float dominantAxis = max(max(absVec.x, absVec.y), absVec.z); // TODO use max3() instead
    ...
    {       
        ...
        if (NGSS_FALLBACK_TO_INTERNAL > 0)
        {
            return UnitySampleShadowmapInternal(vec);
        }
...
fixed UnitySampleBakedOcclusion (float2 lightmapUV, float3 worldPos)
{
    ...
    // Magic Lightmap Switcher
            ...
            //<FTRACEV1.0>
                    ...
                    fixed4 rawOcclusionMask = BlendTwoTextures(2, lightmapUV.xy); //Unity 2017 and below
                #else
                    fixed4 rawOcclusionMask = ftBicubicSampleShadow2(unity_ShadowMask, unity_ShadowMask, lightmapUV.xy); //Unity 2017 and below
                #endif
            #else
            //</FTRACEV1.0>
                #if defined(_MLS_ENABLE_LIGHTMAPS_BLENDING)
                    fixed4 rawOcclusionMask = BlendTwoTextures(2, lightmapUV.xy); //Unity 2017 and below
                #else
                    fixed4 rawOcclusionMask = UNITY_SAMPLE_TEX2D_SAMPLER(unity_ShadowMask, unity_ShadowMask, lightmapUV.xy); //Unity 2017 and below
                #endif
            //<FTRACEV1.0>
            #endif
            //</FTRACEV1.0>
            ...
            fixed4 rawOcclusionMask = fixed4(1.0, 1.0, 1.0, 1.0);
                ...
                if (unity_ProbeVolumeParams.x == 1.0)
                    ...
                    //<FTRACEV1.0>
                    #ifdef USEFTRACE
                    rawOcclusionMask = ftBicubicSampleShadow2(unity_ShadowMask, unity_ShadowMask, lightmapUV.xy);
                    #else
                    //</FTRACEV1.0>
                    rawOcclusionMask = UNITY_SAMPLE_TEX2D(unity_ShadowMask, lightmapUV.xy);
                    //<FTRACEV1.0>
                ...
                #ifdef USEFTRACE
                    #if defined(_MLS_ENABLE_LIGHTMAPS_BLENDING)
                        rawOcclusionMask = BlendTwoTextures(2, lightmapUV.xy); //Unity 2017 and below
                    #else
                        rawOcclusionMask = ftBicubicSampleShadow2(unity_ShadowMask, unity_ShadowMask, lightmapUV.xy); //Unity 2017 and below
                    #endif
                #else
                //</FTRACEV1.0>
                    #if defined(_MLS_ENABLE_LIGHTMAPS_BLENDING)
                        rawOcclusionMask = BlendTwoTextures(2, lightmapUV.xy); //Unity 2017 and below
                    #else
                        rawOcclusionMask = UNITY_SAMPLE_TEX2D(unity_ShadowMask, lightmapUV.xy); //Unity 2017 and below
                    #endif
                //<FTRACEV1.0>
                #endif
                //</FTRACEV1.0>
...
fixed4 UnityGetRawBakedOcclusions(float2 lightmapUV, float3 worldPos)
{
            ...
            #ifdef USEFTRACE
                #if defined(_MLS_ENABLE_LIGHTMAPS_BLENDING)
                    return BlendTwoTextures(2, lightmapUV.xy); //Unity 2017 and below
                #else
                    return ftBicubicSampleShadow2(unity_ShadowMask, unity_ShadowMask, lightmapUV.xy); //Unity 2017 and below
                #endif
            #else
            //</FTRACEV1.0>
                ...
                // Magic Lightmap Switcher
                    ...
                    return BlendTwoTextures(2, lightmapUV.xy); //Unity 2017 and below
                #else
                    return UNITY_SAMPLE_TEX2D(unity_ShadowMask, lightmapUV.xy); //Unity 2017 and below
                #endif
            //<FTRACEV1.0>
            //</MLS_UNITY_SHADOW_LIBRARY_UNITY_GET_RAW_BAKED_OCCLUSIONS_ADDITIONS>
            #endif
...
//<FTRACEV1.0>
#ifdef USEFTRACE
            return realtimeShadowAttenuation * bakedShadowAttenuation;//(realtimeShadowAttenuation * bakedShadowAttenuation);
#else
//</FTRACEV1.0>
