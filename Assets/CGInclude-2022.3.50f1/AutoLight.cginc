...
#   define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
        unityShadowCoord3 lightCoord = mul(unity_WorldToLight, unityShadowCoord4(worldPos, 1)).xyz; \
        fixed shadow = UNITY_SHADOW_ATTENUATION(input, worldPos);\
        fixed destName = ftLightFalloff(unity_WorldToLight, worldPos) * shadow;
//</FTRACEV1.0>
#endif
...
#   define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
        ...
        fixed shadow = UNITY_SHADOW_ATTENUATION(input, worldPos); \
        fixed destName = (lightCoord.z > 0) * UnitySpotCookie(lightCoord) * ftLightFalloff(unity_WorldToLight, worldPos) * shadow;
#endif
...
#   define LIGHT_ATTENUATION(a)    ( (a._LightCoord.z > 0) * UnitySpotCookie(a._LightCoord) * ftLightFalloff(unity_WorldToLight, worldPos) * SHADOW_ATTENUATION(a) )
#endif
